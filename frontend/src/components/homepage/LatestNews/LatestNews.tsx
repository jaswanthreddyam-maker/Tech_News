...
export function LatestNews() {
    ...
    return (
      <section>
        ...
        <div className="flex flex-col gap-6">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="grid grid-cols-1 md:grid-cols-4 gap-6 p-4 -mx-4 rounded-xl">
              <Skeleton level={loadingLevel} className="md:col-span-1 aspect-video w-full" />
              <div className="md:col-span-3 flex flex-col justify-center">
                <div className="flex gap-2 mb-3">
                  <Skeleton level={loadingLevel} className="h-3 w-16" />
                  <Skeleton level={loadingLevel} className="h-3 w-24" />
                </div>
                <Skeleton level={loadingLevel} className="h-7 w-full mb-2" />
                <Skeleton level={loadingLevel} className="h-7 w-3/4 mb-3" />
                <Skeleton level={loadingLevel} className="h-4 w-full mb-2" />
                <Skeleton level={loadingLevel} className="h-4 w-5/6 mb-4" />
                <Skeleton level={loadingLevel} className="h-3 w-32 mt-auto" />
              </div>
            </div>
          ))}
        </div>
  ...
  if (error) return (
    <div className="py-8 mt-8">
      <ErrorState title="Error loading news" description="Could not load the latest intelligence feed." />
    </div>
  );
